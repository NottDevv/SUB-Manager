    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col justify-between p-4 sm:p-6 lg:p-8">
      ...
      <header className="max-w-4xl mx-auto w-full flex items-center justify-between pb-6 border-b border-slate-800">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={onLogout}
            className="p-2 rounded-xl bg-rose-600/90 text-white hover:opacity-75 border border-rose-500/40 transition-all shadow-sm active:scale-95 btn-logout"
            title={t('logout')}
          >
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 shrink-0 self-end sm:self-auto">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-emerald-600/20 hover:bg-emerald-600 text-emerald-300 hover:text-white border border-emerald-500/30 text-xs font-semibold transition-all btn-action-copy"
            >
              {copiedIndex === idx ? (
            ...
            {/* Import to App Dropdown for Subscription and Config Links */}
            <div className="relative inline-block">
              ...
              {showDeepLinks === idx && (
                <div className="absolute top-full end-0 mt-2 w-[220px] bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl p-2 z-30 space-y-1 animate-fadeIn">
                  <a
                    href={isSub ? `v2rayng://install-sub?url=${encodeURIComponent(link.url)}&name=${encodeURIComponent(user.username || "")}` : `v2rayng://install-config?url=${encodeURIComponent(link.url)}`}
                  ...
                  <a
                    href={isSub ? `shadowrocket://add/sub://${btoa(link.url)}?title=${encodeURIComponent(user.username || "")}` : `shadowrocket://add/${link.url}`}
                    className="flex items-center gap-2 px-3 py-2 rounded-xl hover:bg-slate-800 text-xs font-medium text-slate-200 transition-colors"
                  >
                    <ExternalLink className="w-3.5 h-3.5 text-purple-400" />
                    <span>{t('import_shadowrocket')}</span>
                  </a>
                  <a
                    href={`nekobox://import?url=${encodeURIComponent(link.url)}&name=${encodeURIComponent(user.username || "")}`}
                    className="flex items-center gap-2 px-3 py-2 rounded-xl hover:bg-slate-800 text-xs font-medium text-slate-200 transition-colors"
                  >
                    <ExternalLink className="w-3.5 h-3.5 text-blue-400" />
                    <span>{t('import_nekobox')}</span>
                  </a>
                  <a
                    href={`streisand://import?url=${encodeURIComponent(link.url)}&name=${encodeURIComponent(user.username || "")}`}
                    className="flex items-center gap-2 px-3 py-2 rounded-xl hover:bg-slate-800 text-xs font-medium text-slate-200 transition-colors"
                  >
                    <ExternalLink className="w-3.5 h-3.5 text-orange-400" />
                    <span>{t('import_streisand')}</span>
                  </a>
                  <a
                    href={isSub ? `v2box://install-sub?url=${encodeURIComponent(link.url)}&name=${encodeURIComponent(user.username || "")}` : `v2box://install-config?url=${encodeURIComponent(link.url)}`}
                    className="flex items-center gap-2 px-3 py-2 rounded-xl hover:bg-slate-800 text-xs font-medium text-slate-200 transition-colors"
                  >
                    <ExternalLink className="w-3.5 h-3.5 text-pink-400" />
                    <span>{t('import_v2box')}</span>
                  </a>
                </div>
              )}
            </div>
            ...
            <button
              ...
              className="p-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-cyan-400 transition-colors border border-slate-700/60 btn-action-qr"
              title={t('qr_code')}
            >
