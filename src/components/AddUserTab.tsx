...
import { UserPlus, User, Lock, Calendar, Eye, EyeOff, Sparkles } from 'lucide-react';
...
export const AddUserTab: React.FC<AddUserTabProps> = ({ onAddUser }) => {
  const { t } = useLang();
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="space-y-2 pt-2">
            ...
            <input
              ...
              className="w-full px-4 py-3 rounded-xl bg-slate-950 border border-slate-800 text-slate-100 text-sm font-mono focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 focus:outline-none transition-all"
            />
            <p className="text-[11px] text-slate-500 ms-6 text-start">{t('expire_date_hint')}</p>
          </div>
