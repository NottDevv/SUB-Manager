    ...
    <>
      ...
      >
          ...
          {navItems.map((item) => {
            ...
            return (
              <button
                ...
                title={item.label}
                className={`w-full flex items-center gap-3.5 px-3.5 py-3.5 rounded-xl font-medium text-sm transition-all duration-200 relative overflow-hidden group/navbtn ${
                  isActive
              ...
              >
                <div className={`shrink-0 transition-transform duration-300 ${animClass} ${isActive ? 'scale-110' : 'group-hover/navbtn:scale-110'}`}>
                  <Icon
                    className={`w-5 h-5 ${
                      isActive
                        ? 'text-emerald-400'
                        : 'text-slate-400 group-hover/navbtn:text-emerald-400'
                    }`}
                  />
                </div>
        ...
        <div className="p-3 lg:p-4 border-t border-slate-800/80 space-y-3 bg-slate-950/40">
          ...
          {/* Logout Button (Vibrant in all themes, fades on hover) */}
          <button
            onClick={onLogout}
            title={t('logout')}
            className="w-full flex items-center justify-center gap-2.5 p-3 rounded-xl bg-rose-600/90 text-white hover:opacity-75 border border-rose-500/50 transition-all duration-200 font-bold text-sm shadow-md shadow-rose-950/40 overflow-hidden active:scale-95 btn-logout"
          >
            <LogOut className="w-4 h-4 shrink-0 text-white icon-logout" />
